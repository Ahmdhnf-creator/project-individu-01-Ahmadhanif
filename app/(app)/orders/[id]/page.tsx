...
const paymentStatusColors: Record<string,string> = { BELUM_BAYAR:"bg-yellow-200", LUNAS:"bg-green-200" };
  ...
  return (
    <div>
      ...
      <div className="mt-2 flex gap-2 items-center">
        ...
        <span className="text-sm">Bayar: {order.paymentMethod}</span>
        <span className={`rounded px-2 py-1 text-sm ${paymentStatusColors[order.paymentStatus] ?? "bg-gray-200"}`}>{order.paymentStatus}</span>
      </div>
      {order.proofUrl && <a href={getUrl(order.proofUrl)} target="_blank" rel="noopener noreferrer" className="mt-2 inline-block text-sm text-blue-600 underline">Lihat bukti</a>}
      ...
      <PaymentActions orderId={order.id} paymentMethod={order.paymentMethod} paymentStatus={order.paymentStatus} proofUrl={order.proofUrl} />
